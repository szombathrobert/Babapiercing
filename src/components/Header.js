  ...
  return <header className={`${
    ...
    >
      <div className='flex justify-between items-center h-full pl-[50px] pr-[60px]'>
        {/* logo */}
        <a href='/'>
          <img className='w-[188px] h-[90px]' src={logo}></img>
        </a>
        ...
        <div onClick={() => setNavMobile(!navMobile)} className='xl:hidden absolute right-[5%] p-2 rounded-md cursor-pointer'>
          <TiThMenuOutline className='text-4xl'/>
        </div>
        {/* nav mobile - showing by default -hidden on desktop */}
        <div 
          className={`${
            navMobile ? 'max-h-full' : 
            'max-h-0'} ${
              isActive 
